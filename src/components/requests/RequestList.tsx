...
const RequestList: React.FC<Props> = ({ type, view, status }) => {
  ...
  const normalize = (x: any, fallbackMgr: string = "") => {
    ...
    return {
      ...
      ptime: safeText(pick(itemObj, "ptime", "pTime", "PTime", "PTIME")),
      // ✅ APPROVAL
  ...
  const loadData = async (silent: boolean = false) => {
    ...
    try {
      ...
      let result = (Array.isArray(res.data) ? res.data : [])
        ...
        .filter(Boolean)
        .filter((it: any) => {
          // Hide 0-minute dummy permission records generated by database SP, unless it's an explicit LOP record
          if (
            it?.ltype?.toLowerCase() === "permission" &&
            String(it?.ptime).trim() === "0" &&
            !String(it?.LeaveCategory).toLowerCase().includes("lop") &&
            !String(it?.Remarks).toLowerCase().includes("lop")
          ) {
            return false;
          }
          return true;
